...
import { DefaultUserAuthenticationTokenStore } from "../../../../src/core/extensions/user-stores/index.js";
...
describe("DefaultUserAuthenticationTokenStore", () => {
  const store = new DefaultUserAuthenticationTokenStore<string, TestUser>();
  const user = new TestUser("1", "Alice");
  const cancellationToken = new CancellationToken();

  it("setTokenAsync should throw", () => {
    expect(() =>
      store.setTokenAsync(user, "provider", "tokenName", "value", cancellationToken)
    ).toThrow("Method not implemented.");
  });

  it("removeTokenAsync should throw", () => {
    expect(() =>
      store.removeTokenAsync(user, "provider", "tokenName", cancellationToken)
    ).toThrow("Method not implemented.");
  });

  it("getTokenAsync should throw", () => {
    expect(() =>
      store.getTokenAsync(user, "provider", "tokenName", cancellationToken)
    ).toThrow("Method not implemented.");
  });
