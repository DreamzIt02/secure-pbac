...
import { DefaultUserSecurityStampStore } from "../../../../src/core/extensions/user-stores/index.js";
...
describe("DefaultUserSecurityStampStore", () => {
  const store = new DefaultUserSecurityStampStore<TestUser>();
  const user = new TestUser("1", "Alice");
  const cancellationToken = new CancellationToken();

  it("setSecurityStampAsync should throw", () => {
    expect(() => store.setSecurityStampAsync(user, "stamp123", cancellationToken))
      .toThrow("Method not implemented.");
  });

  it("getSecurityStampAsync should throw", () => {
    expect(() => store.getSecurityStampAsync(user, cancellationToken))
      .toThrow("Method not implemented.");
  });
